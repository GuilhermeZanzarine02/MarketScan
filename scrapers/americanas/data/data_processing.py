...
def americanas_handle_data(all_data, file_type, price_order, rows):
    ...
    try:
        if file == 'excel(.xlsx)' and order_file == 'highest to lowest':
            if rows:
                rows = int(rows)
                data = data.sort_values(by='price pix', ascending=False).head(rows)
                path = f"{downloads_path}/data_scraped_highest_to_lowest_price_{timestamp}.xlsx"
                data.to_excel(path, index=False)
            else:
                data = data.sort_values(by='price pix', ascending=False)
                path = f"{downloads_path}/data_scraped_highest_to_lowest_price_{timestamp}.xlsx"
                data.to_excel(path, index=False)
            ...
            if rows:
                rows = int(rows)
                data = data.sort_values(by='price pix', ascending=True).head(rows)
                path = f"{downloads_path}/data_scraped_lowest_to_highest_price_{timestamp}.xlsx"
                data.to_excel(path, index=False)
            else:
                data = data.sort_values(by='price pix', ascending=True)
                path = f"{downloads_path}/data_scraped_highest_to_lowest_price_{timestamp}.xlsx"
                data.to_excel(path, index=False)
